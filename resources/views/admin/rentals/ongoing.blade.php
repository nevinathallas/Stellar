                ...
                <div class="table-responsive">
                    <table class="table table-hover">
                        ...
                        <tbody>
                            ...
                            <tr class="{{ $rental->due_date->isPast() ? 'table-danger' : '' }} {{ $rental->hasPendingReturnRequest() ? 'table-warning' : '' }}">
                                ...
                                <td>
                                    ...
                                    @if($rental->hasPendingReturnRequest())
                                        <br><span class="badge bg-warning text-dark mt-1">
                                            <i class="bi bi-bell"></i> Ada Request
                                        </span>
                                    @endif
                                </td>
                                <td>
                ...
                <div class="modal-content">
                    ...
                    <form action="{{ route('admin.rentals.return', $rental->id) }}" method="POST">
                        ...
                        <div class="modal-body">
                            ...
                            <!-- Bukti Pembayaran dari Member (jika ada) -->
                            @if($rental->hasPendingReturnRequest())
                                <div class="alert alert-warning">
                                    ...
                                    <p class="mb-2">
                                        <strong>Waktu Request:</strong> {{ $rental->return_requested_at->format('d/m/Y H:i') }}
                                    </p>
                                    
                                    @if($rental->payment_proof)
                                        <div class="mb-2">
                                            <strong>Bukti Pembayaran:</strong><br>
                                            <a href="{{ asset('storage/' . $rental->payment_proof) }}" target="_blank" class="btn btn-sm btn-primary mt-1">
                                    ...
                                    @if($rental->notes)
                                        <div>
                                            <strong>Catatan Member:</strong><br>
                                            <em>"{{ $rental->notes }}"</em>
                                        </div>
                                    @endif
                                </div>
                            ...
                            <table class="table table-borderless">
                                <tr>
                                    <td width="40%"><strong>Member:</strong></td>
                            ...
                            @if($rental->hasPendingReturnRequest())
                                <p class="mb-0 text-warning fw-bold">
                                    <i class="bi bi-exclamation-triangle"></i> Member sudah mengirim bukti pembayaran. Verifikasi dan kembalikan planet?
                                </p>
                            @else
                        ...
                        <div class="modal-footer">
                            ...
                            <button type="submit" class="btn btn-success">
                                <i class="bi bi-check-circle"></i> {{ $rental->hasPendingReturnRequest() ? 'Verifikasi & Kembalikan' : 'Ya, Kembalikan' }}
                            </button>
                        </div>
                    </form>
                </div>
